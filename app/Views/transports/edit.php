...
<h1 class="text-3xl font-bold mb-6 text-app-gray-800">Modifier le transport: <?php echo htmlspecialchars($transport['attestation']); ?></h1>
...
<div class="bg-white shadow-md rounded-lg p-6 max-w-2xl mx-auto">
    ...
    <form action="<?php echo BASE_URL; ?>/transports/edit/<?php echo $transport['id']; ?>" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="attestation" name="attestation" value="<?php echo htmlspecialchars($transport['attestation']); ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-app-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>
        <div class="mb-4">
            ...
            <select id="status" name="status" class="shadow appearance-none border rounded w-full py-2 px-3 text-app-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                <option value="Brouillon" <?php echo ($transport['status'] == 'Brouillon') ? 'selected' : ''; ?>>Brouillon</option>
                <option value="En attente" <?php echo ($transport['status'] == 'En attente') ? 'selected' : ''; ?>>En attente</option>
                <option value="Validé" <?php echo ($transport['status'] == 'Validé') ? 'selected' : ''; ?>>Validé</option>
                <option value="Annulé" <?php echo ($transport['status'] == 'Annulé') ? 'selected' : ''; ?>>Annulé</option>
            </select>
        </div>
        <div class="mb-4">
            <label for="date" class="block text-app-gray-700 text-sm font-bold mb-2">Date du transport:</label>
            <input type="date" id="date" name="date" value="<?php echo htmlspecialchars($transport['date']); ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-app-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
        </div>
        <div class="mb-6">
            ...
            <select id="client_id" name="client_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-app-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                ...
                <?php foreach ($clients as $client): ?>
                    <option value="<?php echo htmlspecialchars($client['id']); ?>" <?php echo ($transport['client_id'] == $client['id']) ? 'selected' : ''; ?>>
        ...
        <div id="transport-lines-container">
            <?php if (!empty($transport_lines)): ?>
                <?php foreach ($transport_lines as $idx => $line): ?>
        ...
        <div id="declarations-container">
            <?php if (!empty($transport_declarations)): ?>
                <?php foreach ($transport_declarations as $idx => $declaration): ?>
...
<script>
    let lineIndex = <?php echo !empty($transport_lines) ? count($transport_lines) : 0; ?>;
    let declarationIndex = <?php echo !empty($transport_declarations) ? count($transport_declarations) : 0; ?>;
